...
export class BaseHttpService {
  ...
  protected get<T>(endpoint: string, params?: HttpParams, headers?: HttpHeaders): Observable<T> {
    return this.http.get<ApiResponse<T>>(this.buildUrl(endpoint), { params, headers }).pipe(this.handleResponse(), catchError(this.handleError));
  }

  protected post<T>(endpoint: string, body: any, headers?: HttpHeaders): Observable<T> {
    return this.http.post<ApiResponse<T>>(this.buildUrl(endpoint), body, { headers }).pipe(this.handleResponse(), catchError(this.handleError));
  }

  protected put<T>(endpoint: string, body: any, headers?: HttpHeaders): Observable<T> {
    return this.http.put<ApiResponse<T>>(this.buildUrl(endpoint), body, { headers }).pipe(this.handleResponse(), catchError(this.handleError));
  }

  protected delete<T>(endpoint: string, headers?: HttpHeaders): Observable<T> {
    return this.http.delete<ApiResponse<T>>(this.buildUrl(endpoint), { headers }).pipe(this.handleResponse(), catchError(this.handleError));
  }
